...
import { chatWithFallback, detectProvider, IaError, iaMessage, keyHint, maxTokensFor, parseVariacoes, promptVariacoes, PROVIDERS, type Provider, validarVariacoes } from "./ia.ts";

export const AI_DAILY_LIMIT = 150;
type Db = SupabaseClient;
type ChatFn = typeof chatWithFallback;

const isAdmin = (c: Caller) => c.role === "owner" || c.role === "admin";
...
export async function varyCaptions(db: Db, caller: Caller, body: { caption?: string; count?: number }, chatFn: ChatFn = chatWithFallback) {
  ...
  if (!Number.isFinite(count) || count < 1) throw new HttpError(400, "Diga quantas variações quer.");
  ...
  if ((await aiCallsToday(db, caller.teamId)) >= AI_DAILY_LIMIT) {
    throw new HttpError(429, `Este time já usou as ${AI_DAILY_LIMIT} gerações com IA das últimas 24 horas.`, "limite_ia");
  }
  ...
  try {
    ...
    const variations = validarVariacoes(caption, parseVariacoes(out.text), count);
    await log({ model: out.model, returned: variations.length, ok: true });
    if (out.model !== cfg.model) await db.from("team_ai_keys").update({ model: out.model }).eq("team_id", caller.teamId);
    return { variations, requested: count, provider: cfg.provider, provider_label: PROVIDERS[cfg.provider].label, model: out.model };
  } catch (e) {
    if (!(e instanceof IaError)) throw e;
    await log({ ok: false, error: `${e.kind}: ${e.message}`.slice(0, 300) });
    throw new HttpError(e.kind === "chave" ? 400 : e.kind === "cota" ? 429 : 502, iaMessage(e, cfg.provider), `ia_${e.kind}`);
  }
}
