-- Início: período "Ontem". dashboard() ganha p_ate (último dia do período,
-- padrão = hoje). Os números do período (publicadas, falhas, distribuição,
-- contas) terminam em p_ate; o que olha para a frente (agendadas hoje,
-- próximos 7 dias, ritmo até hoje + 7, uso do mês) continua a partir de hoje.
-- Sem p_ate a resposta é a mesma de antes, então o painel antigo segue igual.
-- Troca a assinatura: derruba a de 2 argumentos para não ficar ambígua.
drop function if exists public.dashboard(uuid, int);

create or replace function public.dashboard(p_team uuid, p_days int default 30, p_ate date default null)
returns jsonb
language sql stable security invoker set search_path = public as $$
with
rng as (
  select today, days, fim,
         fim - (days - 1)     as d0,  -- começo do período
         fim - (2 * days - 1) as p0,  -- começo do período anterior (comparação)
         date_trunc('month', today)::date as m0
  from (select today, days, least(coalesce(p_ate, today), today) as fim  -- fim do período: hoje ou antes
        from (select (now() at time zone 'America/Bahia')::date as today,
                     greatest(1, least(coalesce(p_days, 30), 180))::int as days) c0) c
),
p as (
...
by_platform as (
  ...
  from (select t.platform as k, count(*) as n from t, rng
        where t.status = 'published' and t.done_d between rng.d0 and rng.fim group by 1) x
),
by_placement as (
  select coalesce(jsonb_agg(jsonb_build_object('k', k, 'n', n) order by n desc), '[]'::jsonb) as v
  from (select p.placement as k, count(*) as n from p, rng
        where p.at_local::date between rng.d0 and rng.fim group by 1) x
),
failures as (
  select coalesce(jsonb_agg(jsonb_build_object('k', k, 'n', n) order by n desc), '[]'::jsonb) as v
  from (select left(coalesce(nullif(btrim(t.error), ''), 'Sem detalhe do erro'), 90) as k, count(*) as n
        from t, rng where t.status = 'failed' and t.done_d between rng.d0 and rng.fim
...
accs as (
  ...
  cross join rng
  cross join lateral (
    select count(*) filter (where t.status = 'published' and t.done_d between rng.d0 and rng.fim) as published,
           count(*) filter (where t.status = 'failed' and t.done_d between rng.d0 and rng.fim)    as failed,
...
kpis as (
  select jsonb_build_object(
    ...
    'published',      (select count(*) from t, rng where t.status = 'published' and t.done_d between rng.d0 and rng.fim),
    'failed',         (select count(*) from t, rng where t.status = 'failed' and t.done_d between rng.d0 and rng.fim),
...
select jsonb_build_object(
  'days', rng.days, 'today', rng.today, 'ate', rng.fim,
...
revoke all on function public.dashboard(uuid, int, date) from public, anon;
grant execute on function public.dashboard(uuid, int, date) to authenticated, service_role;
